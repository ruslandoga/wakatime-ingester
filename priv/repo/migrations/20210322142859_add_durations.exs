defmodule Ingester.Repo.Migrations.AddDurations do
  use Ecto.Migration
    ...
    execute """
            create or replace view durations as
              ...
              select time, project, entity, type, branch,language,
                      case when time - lag(time,1) over (order by time) > interval '0 day 00:05:00'
                            then 1
                            else 0
                      end grp_start
                  from heartbeats
              ...
              assign_grp as (
                select time, project, entity, type, branch,language,
                sum(grp_start) over (order by time) duration_id
                from start_grp)
